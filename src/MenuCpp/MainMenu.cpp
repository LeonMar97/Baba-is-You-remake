...
:
	...
	sf::RectangleShape(sf::Vector2f(430, 48))
		, sf::RectangleShape(sf::Vector2f(430, 48)),
		sf::RectangleShape(sf::Vector2f(430, 48)) }
		*/
{

	add(std::make_unique <NewGame>(m_cntrl, std::make_unique <sf::RectangleShape>(sf::RectangleShape(sf::Vector2f(430, 48)))));
...
void MainMenu::draw(sf::Time& deltaTime) {
	...
	m_menuWindow.draw((m_options[START_NEW_GAME])->getRect());
...
void MainMenu::setButtonsTextures() {
	...
	for (auto& curRec : m_options) {
		...
		texRect = sf::IntRect(0, texRect.top + 53, 0, 0);
	}
		...
		while (m_menuWindow.pollEvent(event))
		{
			...
			case sf::Event::MouseButtonPressed:
				...
				if (event.key.code == sf::Mouse::Left) {
					if (m_options[START_NEW_GAME]->contains(mousepos)) {// sf::Vector2f(sf::Mouse::getPosition()))) {
